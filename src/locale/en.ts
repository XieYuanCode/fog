const en = {
  welcome: {
    general: {
      general_setting_title: "General Setting",
      default_cloned_directory_label: "Default Cloned Directory",
      language_label: "Language",
      color_theme_label: "Color Theme",
      color_theme_light: "Light",
      color_theme_dark: "Dark",
      color_theme_system: "System",
    },
    git: {
      git_setting_title: "Git Setting",
      global_name_label: "Global Username",
      global_email_label: "Global Email Address",
      download_button_text: "Go Download",
      git_not_installed_tip: "Git Not Installed! Please Restart The Fog After Installation"
    },
    service_account: {
      service_account_title: "Service Account"
    }
  },
  dialog: {
    select_default_cloned_directory: "select default cloned directory",
    add_service_account: {
      confirm_btn_text: "Confirm",
      cancel_btn_text: "Cancel",
      authenticating_text: "Authenticating",
      authenticate_success_text: "Authenticate Successful",
      authenticate_fail_text: "Authenticate Unsuccessful",
      gitlabceee: {
        host_label_text: "Host",
        name_label_text: "Name",
        pat_label_text: "Personal Access Token"
      },
      github: {
        name_label_text: "Username",
        pat_label_text: "Personal Access Token"
      }
    }
  },
  tooltip: {
    explorer_switch_local_repo_radio: "Switch To Local Repositories Explorer",
    explorer_switch_service_account_radio: "Switch To Service Accounts Explorer",
  }
}

export default en;